    ...
    class RedsysGateway < Gateway
      ...
      def add_external_mpi_fields(data, options)
        ...
        if options[:three_d_secure][:version]
          ...
          # TODO: keep here?
          data[:authenticacionMethod] = options[:three_d_secure][:authentication_method] if options[:three_d_secure][:authentication_method]
          data[:authenticacionType] = options[:three_d_secure][:authentication_type] if options[:three_d_secure][:authentication_type]
          data[:authenticationFlow] = options[:three_d_secure][:authentication_flow] if options[:three_d_secure][:authentication_flow]
